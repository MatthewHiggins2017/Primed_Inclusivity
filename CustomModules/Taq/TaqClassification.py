...
def ThermoCompatibleRegion(PrimerSeq,
	...
	LOO = len(OptimalBindingDict['MaxRelBindngSeq'])

	for Size in list(range(MinSize,LOO+1))[::-1]:

		PossibleShifts = len(OptimalBindingDict['MaxRelBindngSeq']) - Size

		if PossibleShifts == 0:
			OI = [(OptimalBindingDict['MaxRelBindngSeq'],0)]
		else:
			OI = [(OptimalBindingDict['MaxRelBindngSeq'][I:I+Size],I) for I in range(PossibleShifts+1)]

		for PHS, S in OI:
			RegexPassCheck = True
			if (re.search(r'(0)\1', ''.join([str(M) for M in PHS]))):
				RegexPassCheck = False



			if (RegexPassCheck == True) and (MinSize <= Size):

				OptimalBindingDict['ThermoBindingDuplex']=True
				OptimalBindingDict['ThermoBindingSeqs'] = (PrimerSeq[S:S+Size],
														   OptimalBindingDict['FinalSubSeq3to5'][S:S+Size])

				OptimalBindingDict['ThermoPhosphatePresent']  = (len(OptimalBindingDict['ThermoBindingSeqs'][0])*2)-2
				...
				ExcludedRegion = OptimalBindingDict['MaxRelBindngSeq'][:S] + OptimalBindingDict['MaxRelBindngSeq'][S+Size:]
				...
				if ExcludedRegionLen != 0:
					...
					OptimalBindingDict['ThermoReliability'] = (ExcludedRegionMatches /  ExcludedRegionLen) * \
															  (ExcludedRegionLen / len(OptimalBindingDict['MaxRelBindngSeq']))
